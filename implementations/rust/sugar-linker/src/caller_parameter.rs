...
impl CallEdgeV2 {
    ...
    pub fn validate_against(
        ...
        for binding in &self.formal_actual_bindings {
            ...
            let Some(declaration) = contract
                .formal_declarations
                .iter()
                .find(|item| item.coordinate.coordinate_cid == binding.formal_coordinate_cid)
            else {
                return Err(ParameterResolutionGapV1::FormalCoordinateMismatch);
            };
            if declaration.coordinate.ordinal >= contract.formal_declarations.len() {
                return Err(ParameterResolutionGapV1::FormalOrdinalMismatch);
            }
        }
        Ok(())

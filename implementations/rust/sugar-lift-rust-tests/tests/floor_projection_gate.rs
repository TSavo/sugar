...
impl ProjectionLadder {
    fn key(&self) -> String {
        format!("{}:{}:{}", self.file, self.line, self.enclosing_fn)
    }
...
fn collector_names_synthetic_consumer_ladder() {
    ...
    assert!(
        keys.iter()
            .any(|key| key.starts_with("src/sugar/synthetic_consumer.rs:5:consumer")),
        "synthetic consumer ladder should be named by file:line:function; observed={observed:#?}"
    );

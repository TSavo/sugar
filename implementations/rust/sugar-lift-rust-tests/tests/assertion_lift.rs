...
// FIX(a) end-to-end: a recognize-ACCEPTED-but-desugar-BAILED term (an unknown-width
// `wrapping_add` -- `primitive_int` claims it, `wrapping_int_op` returns None) lands
// WARRANTED-UNDECIDED via the factory's opaque-EUF fallback, NOT a refusal. This is the
// over-refusal fix armed by the wrapping producer: the bail degrades to honest dark
// (opaque term, no teeth), never a false dragon.
#[test]
fn wrapping_bail_lands_undecided_not_refused() {
    let src = r#"
        #[test]
        fn t() {
            // `(5)` has no width suffix -> wrapping_int_op can't fold -> desugar bails.
            assert_eq!((5).wrapping_add(3), 8);
        }
    "#;
    let out = lift_file(&parse(src), "coretests/num/wrap_bail.rs");
    assert_eq!(
        out.assertions_refused, 0,
        "a recognized-but-bailed wrapping term must NOT refuse (FIX(a) opaque fallback): {:?}",
        out.skip_reasons
    );
    assert!(
        out.assertions_lifted >= 1,
        "the unknown-width wrapping assertion must lift as warranted-undecided, not refuse: \
         lifted={} skips={:?}",
        out.assertions_lifted,
        out.skip_reasons
    );
    // The bailed term is opaque (no teeth) -- it must NOT have folded to a concrete value
    // (which could false-discharge); it carries the opaque-EUF leaf.
    let doc = sugar_ir_symbolic::serialize::marshal_declarations(&out.decls);
    assert!(
        doc.contains("opaque:"),
        "the bailed wrapping term must be the opaque-EUF fallback leaf: {doc}"
    );
}

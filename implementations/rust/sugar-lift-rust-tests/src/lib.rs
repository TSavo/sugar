...
pub use crate::sugar::source_contract::{
    broad_functional_warrant, emit_value_contract, sig_returns_unit, warrant_conjoined_with_vendor,
    warrant_conjoined_with_vendor_terms,
};
pub(crate) use crate::sugar::source_contract::{lit_membership_term, pattern_membership_formula};
...
fn lift_source_assertion_contracts_in_stmts(
    ...
    for entry in entries {
        match entry.slot {
            SourceAssertionContractSlot::Pre => pre_entries.push(entry.entry),
...
enum SourceAssertionContractSlot {
    Pre,
    Inv,
}

struct SourceAssertionContractEntry {
    entry: AssertionEntry,
    slot: SourceAssertionContractSlot,
}
...
fn source_assertion_entries_in_stmts(
    ...
    reducer: &ReductionCtx<'_>,
    factory_audits: Option<&FactoryAuditLog>,
) -> Vec<SourceAssertionContractEntry> {
    let mut temporal_scope = scope.clone();
    ...
    for stmt in stmts {
        match stmt {
            ...
            Stmt::Macro(stmt_macro) => {
                if matches!(
                    cfg_resolve(&stmt_macro.attrs, options),
                    CfgDisposition::Present
                ) {
            ...
            Stmt::Expr(Expr::Macro(expr_macro), _) => {
                if matches!(
                    cfg_resolve(&expr_macro.attrs, options),
                    CfgDisposition::Present
                ) {
                    collect_source_assertion_contract_expr(
            ...
            Stmt::Expr(expr, _)
                if has_constraint_candidate(expr, &temporal_scope, options, &BTreeMap::new()) =>
            {
                collect_source_assertion_contract_expr(
                    expr,
                    &temporal_scope,
...
fn collect_source_assertion_contract_expr(
    expr: &Expr,
    scope: &TemporalScope,
    options: &LiftOptions,
    reducer: &ReductionCtx<'_>,
    ...
    factory_audits: Option<&FactoryAuditLog>,
    entries: &mut Vec<SourceAssertionContractEntry>,
) {
    if let Some(entry) = sugar::const_bound::lift_const_bound_assertion(expr, scope, factory_audits)
    {
        entries.push(SourceAssertionContractEntry {
            entry,
            slot: SourceAssertionContractSlot::Inv,
        });
        return;
    }
...
pub(crate) fn source_precondition_contains_unresolved_call(formula: &Formula) -> bool {
    match formula {
        Formula::Atomic { args, .. } => args
            .iter()
            .any(|arg| term_contains_unresolved_call(arg.as_ref())),
        Formula::Connective { operands, .. } => operands
            .iter()
            .any(|operand| source_precondition_contains_unresolved_call(operand.as_ref())),
        Formula::Quantifier { body, .. } | Formula::Choice { body, .. } => {
            source_precondition_contains_unresolved_call(body.as_ref())
        }
    }
}
...
pub(crate) fn term_contains_unresolved_call(term: &Term) -> bool {
    match term {
        ...
        Term::Let { bindings, body } => {
            bindings
                .iter()
                .any(|binding| term_contains_unresolved_call(binding.bound_term.as_ref()))
                || term_contains_unresolved_call(body.as_ref())
        }
        Term::Lambda { body, .. } => term_contains_unresolved_call(body.as_ref()),
...
impl MacroRegistry {
    ...
    fn overlay_all(&mut self, other: &MacroRegistry) {
        for (name, rules) in &other.macros {
            if other.ambiguous.contains(name) {
                continue;
            }
            self.insert_rules(name, rules.clone());
        }
    }
...
fn scoped_macro_registry_for_stmts(stmts: &[Stmt], reducer: &ReductionCtx<'_>) -> MacroRegistry {
...
fn scoped_const_registry_for_stmts(stmts: &[Stmt], reducer: &ReductionCtx<'_>) -> ConstRegistry {
    let mut registry = reducer.const_registry_snapshot();
    let mut local = ConstRegistry::new();
    local.scan_stmts(stmts);
    registry.merge_all(&local);
    registry
}
...
fn panic_locus_surface_shape_from_expr(expr: &Expr, scope: &TemporalScope) -> bool {
    match expr {
        Expr::Match(m) => panic_locus_match_survivor(m, scope)
            .is_some_and(|(_, pat)| pattern_has_panic_locus_teeth(pat, scope)),
...
fn is_debug_assert_macro_path(path: &syn::Path) -> bool {
    path.segments
        .last()
        .is_some_and(|seg| seg.ident.to_string().starts_with("debug_assert"))
}

fn debug_assertions_present(options: &LiftOptions) -> bool {
    matches!(
        cfg_resolve_predicate(&CfgPredicate::Name("debug_assertions".to_string()), options),
        CfgDisposition::Present
    )
}
...
pub(crate) fn temporal_panic_freedom_entry(
    ...
    let normal_universe = ctx
        .normal_return_universe_for_subject(&record.expr, temporal_subject.clone())
        .unwrap_or_else(|| not_(atomic_("panic", vec![temporal_subject])));
    let atom = if panics {
        not_(normal_universe)
    } else {
        normal_universe
    };
...
fn rewrite_literal_counter_while_to_for(w: &syn::ExprWhile, scope: &TemporalScope) -> Option<Expr> {
    let (name, end, inclusive) = while_counter_bound(&w.cond)?;
    let start = scope
        .let_bindings_iter()
    ...
    if !stmt_is_unit_increment(increment, &name)
        || body_stmts
            .iter()
            .any(|stmt| stmt_assigns_simple_name(stmt, &name))
    {
        return None;
    }
    let ident = proc_macro2::Ident::new(&name, proc_macro2::Span::call_site());
    let range: Expr = if inclusive {
        syn::parse_quote! { (#start)..=(#end) }
    } else {
        syn::parse_quote! { (#start)..(#end) }
    };
    Some(syn::parse_quote! {
...
fn while_counter_bound(cond: &Expr) -> Option<(String, Expr, bool)> {
    let Expr::Binary(binary) = strip_refs_groups(cond) else {
        return None;
    };
    let name = simple_path_name(&binary.left)?;
    let inclusive = match binary.op {
        BinOp::Lt(_) => false,
        BinOp::Le(_) => true,
        _ => return None,
    };
    Some((name, (*binary.right).clone(), inclusive))
}

fn stmt_is_unit_increment(stmt: &Stmt, name: &str) -> bool {
    match stmt {
        Stmt::Expr(Expr::Binary(binary), _)
            if matches!(binary.op, BinOp::AddAssign(_))
                && simple_path_name(&binary.left).as_deref() == Some(name) =>
        {
            const_int(&binary.right) == Some(1)
        }
        Stmt::Expr(Expr::Assign(assign), _)
            if simple_path_name(&assign.left).as_deref() == Some(name) =>
        {
            let Expr::Binary(rhs) = strip_refs_groups(&assign.right) else {
                return false;
            };
            matches!(rhs.op, BinOp::Add(_))
                && ((simple_path_name(&rhs.left).as_deref() == Some(name)
                    && const_int(&rhs.right) == Some(1))
                    || (simple_path_name(&rhs.right).as_deref() == Some(name)
                        && const_int(&rhs.left) == Some(1)))
        }
        _ => false,
    }
}

fn stmt_assigns_simple_name(stmt: &Stmt, name: &str) -> bool {
    match stmt {
        Stmt::Expr(Expr::Assign(assign), _) => {
            simple_path_name(&assign.left).as_deref() == Some(name)
        }
        Stmt::Expr(Expr::Binary(binary), _)
            if matches!(
                binary.op,
        ...
        Stmt::Expr(Expr::Block(block), _) => block
            .block
            .stmts
            .iter()
            .any(|stmt| stmt_assigns_simple_name(stmt, name)),
        Stmt::Expr(Expr::If(expr_if), _) => {
            expr_if
                ...
                || expr_if
                    .else_branch
                    .as_ref()
                    .is_some_and(|(_, expr)| expr_contains_assignment_to_name(expr, name))
        }
        _ => false,
    }
}

fn expr_contains_assignment_to_name(expr: &Expr, name: &str) -> bool {
    match strip_refs_groups(expr) {
        Expr::Assign(assign) => simple_path_name(&assign.left).as_deref() == Some(name),
        Expr::Binary(binary)
            if matches!(
                binary.op,
                BinOp::AddAssign(_)
...
fn temporal_plan_for_stmts(stmts: &[Stmt], inherited_stateful: &BTreeSet<String>) -> TemporalPlan {
    ...
    // `temporally_unstable` set. A superset of the read-after-gated counter scan above
    // (which is redundant-but-harmless; consolidate the two scans in a follow-up). Closes
...
/// BROADER temporal-instability scan (#19 residual): marks a mut-local TEMPORALLY UNSTABLE
...
/// substitutes them BEFORE `bound_path`, so marking `i` is inert (verified: discharged
/// count unchanged vs baseline). (Consumed-iterator receivers are intentionally left to the
/// existing receiver-ambiguity mechanism -- see the visitor note below.)
fn collect_loop_body_mutated(
    ...
    impl<'ast> syn::visit::Visit<'ast> for LoopBodyV<'_> {
        ...
        // OUT OF SCOPE for this shot: consumed-iterator receivers (`it.next()`/`it.by_ref()`
        // in a loop, then a stale `it.len()`). Marking the receiver `it` here COLLIDES with
        // the existing receiver-ambiguity mechanism (the `loop_receiver_mutation_is_ambiguous_
        // and_skipped` test: that path already skips the `r.next()`/`r.contains()` receiver-
        // identity shape with an "ambiguous temporal identity for receiver" warning, and
        // marking `it` here intercepts it first, suppressing that diagnostic). Whether that
        // mechanism eliminates the FULL `.len()`/by_ref false-refutation class is UNVERIFIED
        // -- so that class is left for a separate focused shot (#19 residual), not claimed
        // handled here. This scan covers loop/closure-body MUTATION only.
...
fn const_macro_path_panic_locus_entry(
    subject: &Expr,
    pat: &syn::Pat,
    scope: &TemporalScope,
) -> Option<AssertionEntry> {
    if !matches!(strip_refs_groups(subject), Expr::Macro(_))
        && !matches!(strip_pat_ref_paren(pat), syn::Pat::Macro(_))
    {
        return None;
    }
    let subject_key = const_path_key_from_expr(subject, scope, 0)?;
    let pat_key = const_path_key_from_pat(pat, scope, 0)?;
    if subject_key != pat_key {
        return None;
    }
    let lhs = str_const(format!("const::{subject_key}"));
    let rhs = str_const(format!("const::{pat_key}"));
    Some(assertion_entry_from_eq(lhs, rhs, scope))
}

fn const_path_key_from_expr(expr: &Expr, scope: &TemporalScope, depth: usize) -> Option<String> {
    if depth > MAX_MACRO_EXPANSION_DEPTH {
        return None;
    }
    match strip_refs_groups(expr) {
        Expr::Path(path) => const_like_path_key(&const_path_key(&path.path)?),
        Expr::Macro(m) => expanded_const_path_key(&m.mac.path, &m.mac.tokens, scope, depth + 1),
        _ => None,
    }
}

fn const_path_key_from_pat(pat: &syn::Pat, scope: &TemporalScope, depth: usize) -> Option<String> {
    if depth > MAX_MACRO_EXPANSION_DEPTH {
        return None;
    }
    match strip_pat_ref_paren(pat) {
        syn::Pat::Path(path) => const_like_path_key(&const_path_key(&path.path)?),
        syn::Pat::Macro(m) => expanded_const_path_key(&m.mac.path, &m.mac.tokens, scope, depth + 1),
        _ => None,
    }
}

fn expanded_const_path_key(
    path: &syn::Path,
    tokens: &proc_macro2::TokenStream,
    scope: &TemporalScope,
    depth: usize,
    ...
    let expanded = macro_expand::expand(&rules, tokens.clone()).ok()?;
    if let Ok(expr) = syn::parse2::<Expr>(expanded.clone()) {
        return const_path_key_from_expr(&expr, scope, depth);
    }
    let parser = syn::Pat::parse_single;
    if let Ok(pat) = Parser::parse2(parser, expanded.clone()) {
        return const_path_key_from_pat(&pat, scope, depth);
    }
    if let Ok(path) = syn::parse2::<syn::Path>(expanded) {
        return const_like_path_key(&const_path_key(&path)?);
    }
    None
}

fn const_like_path_key(key: &str) -> Option<String> {
...
fn pattern_has_panic_locus_teeth(pat: &syn::Pat, scope: &TemporalScope) -> bool {
    if pattern_variant_path(pat).is_some() {
        return true;
    }
    scalar_pattern_has_teeth(pat)
        || tuple_pattern_has_teeth(pat)
        || (matches!(strip_pat_ref_paren(pat), syn::Pat::Macro(_))
            && const_path_key_from_pat(pat, scope, 0).is_some())
}
...
fn panic_locus_subject_refusal_reason(
    ...
    if !pattern_has_panic_locus_teeth(pattern, scope)
...
mod lifter_key_tests {
    ...
    #[test]
    fn callsite_lifts_panic_free_as_not_panic_without_scalar_assertions() {
        // A bare callsite in a passing test asserts that the call returned normally.
        // This is panic-path vocabulary, not a scalar comparison and not a method
        // name whitelist.
        let src = r#"
    ...
    #[test]
    fn panic_free_support_uses_rewritten_method_subject_as_not_panic() {
        // Panic-free is the negation of the panic-path constraint. Its subject must be the
    ...
    #[test]
    fn unconditional_panic_macro_emits_panic_and_not_panic() {
        ...
        // panic path and its negation, giving the solver a real UNSAT signal.
        let src = r#"
            #[test]
            fn panics() {
                panic!("bad");
        ...
        assert_eq!(
            out.assertions_lifted, 0,
            "panic-path contradictions do not increment the scalar assertion counter: {:?}",
            out.decls
        );
        let dump = format!("{:?}", out.decls);
        assert!(
            dump.contains("panic") && dump.contains("not"),
            "panic! should emit a contradictory panic-path formula: {dump}"
        );
    }
    ...
    #[test]
    fn nested_callsite_in_assertion_lifts_not_panic_support() {
    ...
    fn closure_mut_receiver_marks_local_ambiguous() {
        ...
        let s_asserts_conjoined = out.decls.iter().any(|d| {
            d.name.contains("::assertion") && {
                ...
                d.name.contains("s")
                    && !d.name.contains("test_")
                    && !inv.contains("Atomic { name: \"panic\"")

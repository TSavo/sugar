...
// `vec![a, b, c]` and `Vec::from([a, b, c])` / `Vec::from(&[a, b, c])` -- recognized as the
// literal sequence they construct. Each is value-identical to the array literal `[a, b, c]`,
// so it grounds to the SAME `Seq` floor (and element-wise teeth) `[a, b, c]` already does:
// the constructor is NORMALIZED to its equivalent array `Expr` and delegated to the shared
// `LiteralSugar`. No new element/length machinery -- the array path const-evaluates each
// element, so a non-literal element (`vec![compute(), 2]`) stays symbolic for that element
// (finite-or-refuse), exactly as a written `[compute(), 2]` would.
//
...
// `Expr::Repeat` and so declines it here. Likewise `Vec::from(<runtime>)` (a non-array arg)
// is declined -- only a written array argument is a finite literal sequence.
...
/// `vec![a, b, c]` and `Vec::from([a, b, c])` / `Vec::from(&[a, b, c])` construct exactly the
/// array `[a, b, c]`; returning that array lets the shared `LiteralSugar`,
/// `is_literal_sequence_base`, and the static-length path treat the constructor identically to
/// a written array literal. The `vec![x; n]` repeat form normalizes to `Expr::Repeat` and is
/// declined (cardinality is the `array_repeat` shape, not a written element list).
pub(crate) fn collection_literal_array(expr: &Expr) -> Option<Expr> {
    match strip_refs_groups(expr) {
        ...
        // `Vec::from([a, b, c])` / `Vec::from(&[a, b, c])` -- the single argument IS the array.
        Expr::Call(call) if is_vec_from_callee(&call.func) && call.args.len() == 1 => {

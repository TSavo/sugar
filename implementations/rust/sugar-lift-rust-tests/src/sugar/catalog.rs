...
use crate::sugar::{
    array_repeat, array_term, assign_op, await_term, binop, block_term, bool_bitwise, bool_method,
    bound_path, call, cast_term, chain, char_method, char_range_collect_string,
    ...
    duration_accessor, enumerate, field_term, filter, filter_map, flat_map, flatten,
    float_refinement, fold, for_each, for_replay, forall_loop, format_args, format_macro,
    from_bool, function_map, identity_map, impl_method, index, infinity_eq, inspect, int_pow,
    int_sqrt, integer_decode, intersperse_collect_string, intersperse_concat, is_empty, is_sorted,
    iter_next, iter_terminal, iterator, kmerge, len, literal, literal_iterator_quantifier,
    literal_slice, macro_term, map, match_node, match_scrutinee, matches_macro, maybe_uninit_new,
    maybe_uninit_zeroed, method, monadic, nonzero, offset_of, option_adaptor, option_predicate,
    option_unwrap, partition_point, path, peekable, primitive_int, range_accessor, range_contains,
    range_term, raw_addr_term, reference_sequence, reference_term, regex_match, repeat_term,
    result_predicate, result_transpose_collect, rev, size_hint, sizeof, skip, skip_while,
    slice_accessor, slice_index, slice_search, statement_async_future, statement_control_flow,

...
// subctors). It digs each child to its `Term` in source order and emits
// `Term::Ctor { name, args }`. A child `Hit` propagates verbatim; a non-term child
// digs to the structural backstop (`from_opt(None)`). This was an inline node inside

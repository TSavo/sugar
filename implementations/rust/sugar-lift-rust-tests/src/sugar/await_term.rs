...
// child. Recognition captures the raw base expression; `desugar` builds the child
// lazily. Byte-identical to the `Expr::Await` arm of the old fat factory.
...
pub(crate) fn recognize(expr: &Expr, fcx: &SugarBuildCtx) -> Option<Box<dyn Sugar>> {
    match expr {
        Expr::Await(await_expr) => Some(Box::new(AwaitTermSugar {
            base: await_expr.base.as_ref().clone(),
...
impl Sugar for AwaitTermSugar {
    fn desugar(&self, ctx: &SugarCtx) -> Outcome {
        let stable = crate::sugar::format::stable_let_bindings(ctx.scope);
        let let_inits = merge_let_inits(&stable, &self.let_inits);
        let fcx = SugarBuildCtx::new(ctx.scope, ctx.options, &let_inits);
        CtorSugar::new("await", vec![build_term(&self.base, &fcx)]).desugar(ctx)
    }
}

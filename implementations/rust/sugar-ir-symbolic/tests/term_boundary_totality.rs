...
//
// IrTerm boundary-collapse campaign (#3191), Slice 1 Instrument A.
//
// Slice 2 will add sugar-walk's real term_boundary wrappers. This slice pins
// the existing converter directly so later wiring has executable gravity:
// `raise(lower(x)) == x` byte-identically for the corpus, and the known lossy
// sort seam refuses loudly instead of silently succeeding.

use std::panic;

use sugar_ir_symbolic::convert::{formula_from_ir, formula_to_ir, term_from_ir, term_to_ir};
use sugar_ir_symbolic::ir_types as ir;
...
#[test]
fn ir_terms_round_trip_through_existing_convert_byte_identically() {
    let corpus = term_corpus();
    for (label, original) in &corpus {
        let lowered = term_from_ir(original.clone());
        let raised = term_to_ir(&lowered);
        assert_same_term_json_bytes(label, original, &raised);
    }
    eprintln!(
        "R(term-round-trip-byte-drift) = 0 corpus_terms={}",
        corpus.len()
    );
}

#[test]
fn ir_formulas_round_trip_through_existing_convert_byte_identically() {
    let corpus = formula_corpus();
    for (label, original) in &corpus {
        let lowered = formula_from_ir(original.clone());
        let raised = formula_to_ir(&lowered);
        assert_same_formula_json_bytes(label, original, &raised);
    }
    eprintln!(
        "R(formula-round-trip-byte-drift) = 0 corpus_formulas={}",
        corpus.len()
    );
}
...
#[test]
fn unsupported_sorts_are_loud_refusals_not_silent_successes() {
    ...
    for expected in EXPECTED_CONVERSION_REFUSALS {
        ...
        let refusal = panic::catch_unwind(|| term_from_ir(term));

...
//
// #3809: assertion level of the typed descent = SourceMemento[path].
//
// - Path is the typed-descent address: file[fn[assertion]]
// - Factory 1:1: assertion path ≡ owning call-site path (same leaf span)
// - Memento CIDs remain the sealed wire currency (fragment stays local)
// - CallSite::assertions stamps SourceMementoAtPath on every Assertion
//
...
#[test]
fn assertions_expose_source_memento_at_path() {
    ...
    for function in &functions {
        ...
        for site in function.call_sites().expect("call_sites") {
            ...
            // Factory 1:1 — one assertion per call site.
            assert_eq!(
                assertions.len(),
                1,
                "factory 1:1: one assertion per site, got {}",
                assertions.len()
            );
            for assertion in &assertions {
                let at = assertion.memento_at_path();
                // Path is the address: file[fn[assertion]]
                ...
                assert!(
                    ...
                    "assertion path has leaf segment: {}",
                    at.path_display()
                );
                // Same nested address as the owning call site (factory 1:1).
                assert_eq!(
                    at.path, site_at.path,
                    "assertion path ≡ call-site path (factory 1:1)"
                );
                assert_eq!(
                    at.path_display(),
                    site_at.path_display(),
                    "path display matches site"
                );
                // Sealed memento is the wire currency (CIDs / span on SourceMemento)
                assert_eq!(at.memento.file, assertion.source_memento().file);
                assert!(
                ...
                found_leaf = true;
                eprintln!(
                    "RECEIPT assertion SourceMemento[path]={path} memento.file={file}",
    ...
    assert!(
        found_leaf,
        "expected at least one assertion with path leaf"
    );
}

#[test]
fn assertions_partitioned_by_enclosing_function() {
    if !python_blake3_available() {
        eprintln!("skip: python3/blake3 unavailable");
        return;
    }
    ...
    for function in &functions {
        ...
        for site in function.call_sites().expect("call_sites") {
            for assertion in site.assertions().expect("assertions") {
                paths.push(assertion.memento_at_path().path_display());
            }
        }
        by_fn.insert(name, paths);
    }
    eprintln!("RECEIPT assertions by function: {by_fn:?}");
    // Each test function owns only its own assertion paths.
    if let Some(a) = by_fn.get("test_a") {
        assert!(!a.is_empty(), "test_a must own at least one assertion");
        for p in a {
            assert!(
                p.contains("test_a"),
                "test_a assertion path must name test_a: {p}"
            );
            assert!(
                !p.contains("test_b"),
    ...
    if let Some(b) = by_fn.get("test_b") {
        assert!(!b.is_empty(), "test_b must own at least one assertion");
        for p in b {
            assert!(
                p.contains("test_b"),
                "test_b assertion path must name test_b: {p}"
            );
        }
    }
}
...
#[test]
fn assertion_path_display_matches_site_leaf() {
    ...
    // Factory 1:1 — assertion leaf uses the same span as the site.
    ...
    let assertion_path = MementoPath::for_site_under(&fn_m, &leaf_m);
    assert_eq!(site_path, assertion_path, "site ≡ assertion path shape");
    let d = assertion_path.display();
    assert!(d.starts_with("a.py[f"), "got {d}");
    assert!(d.contains("2:4-2:20") || d.contains('['), "got {d}");
    eprintln!("RECEIPT assertion path display = {d}");
}

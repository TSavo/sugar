...
///
/// #evict-1-bind: this is a deliberate, byte-identical fork of
/// `sugar_walk::bind::strip_realize_sidecar_from_lift_term`. `bind.rs`
/// moved to `sugar-walk` (toward its consumers there); `lift_plugin.rs`
/// stays in `libsugar` for this step, and `libsugar` must not gain a
/// dependency on `sugar-walk` (that would reverse the dependency arrow
/// sugar-walk -> libsugar into a cycle). The two copies are kept in sync
/// by hand until `lift_plugin.rs` itself relocates toward sugar-cli, at
/// which point this fork can be deleted in favor of the shared one.
fn strip_realize_sidecar_from_lift_term(term: Term) -> Term {
    let Term::Const { mut value, sort } = term else {
        return term;
    };
    if let Some(entries) = value.get_mut("ir").and_then(Value::as_array_mut) {
        for entry in entries {
            if let Some(object) = entry.as_object_mut() {
                ...
                if object.get("kind").and_then(Value::as_str) == Some("bind-lift-entry") {

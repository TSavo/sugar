...
mod tests {
    ...
    /// Issue #3864(c) receipt, redesigned per T's correction: the universe is
    /// OPERATOR-level, not instance-level. The python kit's `_builtin_call_universe`
    /// mints ONE `call:len::universe` FunctionContract per lift (deduped by symbol),
    /// stating len's DOMAIN axiom (`out >= 0`) -- not any call's instance value. The
    /// per-instance equality (`len(x) == N`) stays exactly where it already lived,
    /// as an ordinary ground `EqualityFact`/consumer assertion (the 'Vendor fact'
    /// line these tests exercise everywhere else in this file). This dissolves the
    /// cross-attachment hazard the first (wrong) design created: ONE universe
    /// legitimately attaching to EVERY len callsite is correct -- that IS what a
    /// universe is -- because its post claims only the domain (`out >= 0`), which is
    /// true regardless of which callsite specializes it, so it can never conflict
    /// with (or falsely rescue) any instance fact. Two callsites over DIFFERENT
    /// arguments both link the SAME universe; a bad twin lying about one callsite's
    /// value is still refuted by the conflicting ground fact, same mechanism as
    /// every other same-symbol bridge test in this file.
    #[test]
    fn len_builtin_universe_is_operator_level_and_attaches_to_every_callsite() {
        let (plan, reg) = z3_plan_and_registry();
        let call_len = |arg: Json| json!({"kind":"ctor","name":"call:len","args":[arg]});
        let ge = |a: Json, b: Json| json!({"kind":"atomic","name":"≥","args":[a,b]});

        // ONE operator-level universe for "len": arity matches len's one argument,
        // but the post states only the DOMAIN axiom (non-negative Int), independent
        // of the formal -- exactly what `_builtin_call_universe` now mints, once,
        // regardless of how many len callsites the lift observes.
        let universe_cid = test_cid_string("len-domain-universe");
        let domain_post = ge(var("out"), int(0));

        let mut pool = MementoPool::default();
        pool.insert_unanchored_for_tests(
            test_cid(&universe_cid),
            json!({"evidence": {"kind": "contract", "body": {
                "contractName": "len::builtin-universe",
                "formals": ["x"],
                "outBinding": "out",
                "post": domain_post
            }}}),
        );
        let bridge = json!({"evidence": {"kind": "bridge", "body": {
            "sourceSymbol": "len",
            "targetContractCid": universe_cid.clone(),
            "targetProofCid": "blake3-512:len-universe-proof"
        }}});
        pool.insert_bridge_by_symbol("len", test_cid("len-universe-bridge"), bridge);

        // Two DIFFERENT callsites (different arguments) both link the SAME universe.
        // Deliberately only these two members in the pool -- a third, unrelated
        // member surfaced a separate ground-callsite-fact grouping question (see
        // the note below this test) that is orthogonal to whether the universe
        // itself is safe, so it's kept out of this arm.
        insert_contract(
            &mut pool,
            "blake3-512:good-len-a",
            "src/lib.rs::tests::fresh_len_a::len#euf#c:call:len(i:100)::assertion",
            eqf(call_len(int(100)), int(0)),
        );
        insert_contract(
            &mut pool,
            "blake3-512:good-len-b",
            "src/lib.rs::tests::fresh_len_b::len#euf#c:call:len(i:200)::assertion",
            eqf(call_len(int(200)), int(5)),
        );
        ...
        assert_eq!(res.len(), 2, "two fresh len consumer assertions: {res:?}");

        // The domain universe attaches to and discharges a callsite whose stated
        // value already satisfies out>=0 -- linkedPosts fires, conjoining
        // (universe && instance fact) with no contradiction.
        let good_a = res
            .iter()
            .find(|r| r.contract_cid == test_cid_string("blake3-512:good-len-a"))
            .expect("good len(100) row present");
        assert_eq!(
            good_a.verdict,
            ObligationVerdict::Discharged,
            "len(100)==0 must discharge: the domain universe (out>=0) is consistent \
             with a non-negative stated value: {res:?}"
        );
        let good_a_verification = good_a
            .verification
            .as_ref()
            .expect("good_a carries verification detail")
            .to_json();
        assert_eq!(
            good_a_verification["linkedPosts"][0]["sourceSymbol"],
            "len",
            "the ambient len universe must be linked into this callsite: \
             {good_a_verification:?}"
        );
        assert_eq!(good_a_verification["finalVerdict"], "discharged");

        // The SAME universe attaches to the OTHER (different-argument) callsite too
        // -- that's the correct, intended behavior for an operator-level universe,
        // not a cross-attachment bug: len(200)==5 also satisfies out>=0.
        let good_b = res
            .iter()
            .find(|r| r.contract_cid == test_cid_string("blake3-512:good-len-b"))
            .expect("good len(200) row present");
        assert_eq!(
            good_b.verdict,
            ObligationVerdict::Discharged,
            "len(200)==5 must discharge against the SAME operator-level universe: {res:?}"
        );
        let good_b_verification = good_b
            .verification
            .as_ref()
            .expect("good_b carries verification detail")
            .to_json();
        assert_eq!(
            good_b_verification["linkedPosts"][0]["targetContractCid"],
            universe_cid,
            "both callsites link the identical universe contract, by design: \
             {good_b_verification:?}"
        );
    }

    /// Bad-twin arm: two contracts naming the SAME callsite property (the
    /// `mixed_sort_conjunction_is_named_undecidable`/same-name-conjoin shape this
    /// file already uses elsewhere for "two claims about one obligation") with
    /// CONFLICTING stated values for `len(100)`, in the presence of the domain
    /// universe. The discrimination must come ONLY from the conflicting values, not
    /// from any interaction with the universe -- `out>=0` is satisfied by both 0 and
    /// 2, so it can neither cause nor mask this refutation; the sibling test above
    /// already proves the universe attaches harmlessly to every callsite regardless
    /// of its value.
    ///
    /// NOTE, reported rather than silently dropped: an earlier draft tried to prove
    /// this same claim via TWO SEPARATE consumer assertions (different property
    /// names/scopes, same call term) instead of one shared name, expecting
    /// `verify_consistency`'s cross-contract ground-callsite-fact aggregation
    /// (`collect_ambient_ground_callsite_facts`, doc comment: "a pure g(2) has one
    /// value pool-wide") to conjoin them. It did not behave predictably: with three
    /// contracts in the pool (two different-argument good facts plus a bad twin
    /// sharing an argument with one of them) the bad twin's `vendor_fact_ir` picked
    /// up the OTHER, non-matching-argument fact instead of the matching one and came
    /// back Discharged; with exactly two contracts (good/bad sharing the SAME
    /// argument, mirroring this test) BOTH came back `vendor_fact_ir: None` and both
    /// Discharged -- the cross-contract aggregation didn't fire at all for either
    /// shape tried. That looks like a pre-existing scope/grouping question in
    /// `verify_consistency`'s ground-callsite-fact path, unrelated to this
    /// operator-level-universe design (it is a separate code path from
    /// `collect_ambient_posts`/`linked_ambient_post_instances_for_inv`, which this
    /// change actually touches). Flagged for T/the coordinator rather than chased
    /// further here; this test instead proves the SAME bad-twin claim through the
    /// same-name conjoin mechanism this file already trusts elsewhere.
    #[test]
    fn len_bad_twin_refuted_by_conflicting_fact_domain_universe_is_neutral() {
        let (plan, reg) = z3_plan_and_registry();
        let call_len = |arg: Json| json!({"kind":"ctor","name":"call:len","args":[arg]});
        let ge = |a: Json, b: Json| json!({"kind":"atomic","name":"≥","args":[a,b]});

        let universe_cid = test_cid_string("len-domain-universe-bad-twin");
        let domain_post = ge(var("out"), int(0));
        ...
        let bridge = json!({"evidence": {"kind": "bridge", "body": {
            "sourceSymbol": "len",
            "targetContractCid": universe_cid.clone(),
            "targetProofCid": "blake3-512:len-universe-proof-bad-twin"
        }}});
        pool.insert_bridge_by_symbol("len", test_cid("len-universe-bridge-bad-twin"), bridge);

        // Same callsite property name -- the vendor's own stated len(100)==0 and a
        // lying consumer's len(100)==2 about the identical obligation.
        let name = "src/lib.rs::tests::fresh_len_bad_twin::len#euf#c:call:len(i:100)::assertion";
        insert_contract(
            &mut pool,
            "blake3-512:good-len-vendor",
            name,
            eqf(call_len(int(100)), int(0)),
        );
        insert_contract(
            &mut pool,
            "blake3-512:bad-len-consumer",
            name,
            eqf(call_len(int(100)), int(2)),
        );
        ...
        assert_eq!(
            res[0].verdict,
            ObligationVerdict::Unsatisfied,
            "len(100)==2 must be refuted by the conflicting len(100)==0 vendor fact, \
             independent of the (satisfied-either-way) domain universe: {res:?}"
        );
    }

...
# Intern tables are keyed by finite structural tuples, never by Term/Formula
# objects: frozen-dataclass ``__hash__`` walks ctor/connective spines recursively
# and SEGVs on cyclic graphs (sklearn fenwick / #5340). Keys for compound nodes
...
_FORMULA_INTERN_TABLE: ContextVar[dict[tuple[object, ...], "Formula"] | None] = (
    ContextVar("sugar_formula_intern_table", default=None)
)
...
def _intern_formula(formula: "Formula") -> "Formula":
    ...
    if table is None:
        return formula
    # Formulas are interned bottom-up by their constructors; a single-level
    # structural key avoids recursive dataclass hashing of deep connective spines.
    if isinstance(formula, _Atomic):
        # Terms inside atoms are already interned by term constructors.
        key: tuple[object, ...] = (
            "atomic",
            formula.name,
            tuple(id(arg) for arg in formula.args),
        )
    elif isinstance(formula, _Connective):
        key = (
            "connective",
            formula.kind,
            tuple(id(operand) for operand in formula.operands),
        )
    elif isinstance(formula, _Quantifier):
        key = (
            "quantifier",
            formula.kind,
            formula.name,
            getattr(formula.sort, "name", formula.sort),
            id(formula.body),
        )
    else:
        raise TypeError(f"unknown Formula for intern: {type(formula)!r}")

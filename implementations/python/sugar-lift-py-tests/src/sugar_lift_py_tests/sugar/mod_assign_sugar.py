...
class ModAssignSugar(Sugar, role=SugarRole.STATEMENT):
    """`x %= v`: a rebind over the old binding. Reduce v, read the old x
    (definition-scope law), ask it to modulo, rebind the name to the answer.
    ...
    def owns(cls, site) -> bool:
        if site.observed != "AugAssign" or site.aug_assign_op() != "Mod":
            return False
        return site.aug_assign_target().observed == "Name"

    @classmethod
    def new(cls, site, ctx) -> "ModAssignSugar":
        return cls(
    ...
    def witnesses(cls):
        prefix = "def A(z):\n    x = 7\n    x %= 3\n    return x\n\n"
        return _call_pair(
            name="mod_assign_return",
            owner_sugar="ModAssignSugar",
            truthful=prefix + "def test_a():\n    assert A(5) == 1\n",
            lying=prefix + "def test_a():\n    assert A(5) == 2\n",
        )

    def desugar(self, ctx: object = None) -> Outcome:
        # Reduce v, read the old binding, modulo, rebind to the answer.
        return self.value.reduce(ctx).and_then(
            lambda arg: ctx.temporal.value_for(self.target_name)
            .answer(ctx)
            .and_then(
                lambda old: old.modulo(arg, self.site).and_then(

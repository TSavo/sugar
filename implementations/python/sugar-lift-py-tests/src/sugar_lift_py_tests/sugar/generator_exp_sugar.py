...
class GeneratorExpSugar(Sugar, role=SugarRole.TERM):
    """A generator expression ``(<elt> for <target> in <iter> (if <cond>)*)``.
    ...
    ``py.genexp(elt_term, iter_term, *cond_terms)``. Mirror of ListCompSugar.
    """
    ...
    def owns(cls, site) -> bool:
        if site.observed != "GeneratorExp":
            return False
        generators = site.genexp_generators()
        if len(generators) != 1:
            return False
        gen = generators[0]

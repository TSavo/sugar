...
class DivAssignSugar(Sugar, role=SugarRole.STATEMENT):
    """`x /= v`: a rebind over the old binding. Reduce v, read the old x
    (definition-scope law), ask it to divide, rebind the name to the answer.
    ...
    def owns(cls, site) -> bool:
        if site.observed != "AugAssign" or site.aug_assign_op() != "Div":
            return False
        return site.aug_assign_target().observed == "Name"

    @classmethod
    def new(cls, site, ctx) -> "DivAssignSugar":
        return cls(
    ...
    def witnesses(cls):
        prefix = "def A(z):\n    x = 6\n    x /= 2\n    return x\n\n"
        return _call_pair(
            name="div_assign_return",
            owner_sugar="DivAssignSugar",
            truthful=prefix + "def test_a():\n    assert A(5) == 3\n",
            lying=prefix + "def test_a():\n    assert A(5) == 4\n",
        )

    def desugar(self, ctx: object = None) -> Outcome:
        # Reduce v, read the old binding, divide, rebind to the answer.
        return self.value.reduce(ctx).and_then(
            lambda arg: ctx.temporal.value_for(self.target_name)
            .answer(ctx)
            .and_then(
                lambda old: old.divide(arg, self.site).and_then(

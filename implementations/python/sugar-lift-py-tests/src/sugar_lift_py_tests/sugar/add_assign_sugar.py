...
class AddAssignSugar(Sugar, role=SugarRole.STATEMENT):
    """`x += v`: a rebind over the old binding. Reduce v, read the old x
    (definition-scope law), ask it to add, rebind the name to the answer.
    ...
    @classmethod
    def new(cls, site, ctx) -> "AddAssignSugar":
        return cls(
    ...
    def witnesses(cls):
        prefix = "def A(z):\n    x = 1\n    x += 2\n    return x\n\n"
        return _call_pair(
            name="add_assign_return",
            owner_sugar="AddAssignSugar",
            truthful=prefix + "def test_a():\n    assert A(5) == 3\n",
            lying=prefix + "def test_a():\n    assert A(5) == 4\n",
        )

    def desugar(self, ctx: object = None) -> Outcome:
        # Reduce v, read the old binding, add, rebind to the answer.
        return self.value.reduce(ctx).and_then(
            lambda arg: ctx.temporal.value_for(self.target_name)
            .answer(ctx)
            .and_then(
                lambda old: old.add(arg, self.site).and_then(

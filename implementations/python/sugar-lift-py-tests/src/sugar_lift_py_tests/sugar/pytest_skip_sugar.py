...
class PytestSkipSugar(
    Sugar,
    role=SugarRole.TERM,
    comes_before=("KeywordCallSugar", "MethodCallSugar"),
):
    """The exact ``pytest.skip(...)`` call is a constructed exceptional exit.

    ``pytest.skip`` always raises ``pytest.skip.Exception`` after evaluating
    its arguments. It is terminal control-flow data, not runtime dependence
    and not an opaque method-call value.
    """
    ...
    def witnesses(cls):
        source = (
            "def A(z):\n"
            "    if z < 0:\n"
            "        pytest.skip('negative')\n"
            "    return z\n"
            "\n"
        )
        return _call_pair(
            name="pytest_skip_exceptional_exit",
    ...
    def _reduce_args(
        ...
        exception = ExceptionValue("pytest.skip.Exception", accumulated, site=self.site)
        return Complete(
            RaiseValue(
                RaiseEffect(

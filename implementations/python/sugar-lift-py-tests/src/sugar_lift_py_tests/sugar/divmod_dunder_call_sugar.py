...
class DivmodDunderCallSugar(Sugar, role=SugarRole.TERM, comes_before=("CallSugar",)):
    ...
    def owns(cls, site) -> bool:
        return site.observed == "Call" and site.call_receiver() is None and site.call_target_name() == "divmod" and site.call_arg_count() == 2 and not site.call_has_keywords()

    @classmethod
    def new(cls, site, ctx):
        left, right = site.call_args()
        return cls(ctx.build_body(left, SugarRole.TERM), ctx.build_body(right, SugarRole.TERM), site)

    @classmethod
    def witnesses(cls):
        prefix = "class Box:\n    def __divmod__(self, other):\n        return 1\n\ndef A():\n    return divmod(Box(), 2)\n\n"
        return _call_pair(name="divmod_dunder_return", owner_sugar=cls.__name__, truthful=prefix+"def test_a():\n    assert A() == 1\n", lying=prefix+"def test_a():\n    assert A() == 2\n")

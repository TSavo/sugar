...
def _bind_real_actuals(signature, manager_value):
    ...
    for index, parameter in enumerate(signature.parameters):
        value = None
        present = False
        if positional and isinstance(parameter.passing, (PositionalOnlyV1, PositionalOrKeywordV1)):
            value, present = positional.pop(0), True
            if parameter.name in keywords:
                raise ContextManagerContractError("formal receives positional and keyword actuals")
        elif parameter.name in keywords and isinstance(parameter.passing, (PositionalOrKeywordV1, KeywordOnlyV1)):
    ...
    if positional or keywords:
        raise ContextManagerContractError("call actual does not fit authenticated signature")
    return fixed
...
def _matches_raise(effect, expected, pattern) -> bool:
    ...
    expected_term = expected.to_term(owner="effect-boundary expected type")
    expected_name = getattr(expected_term, "name", None)
    raised = effect.raised_value
    if expected_name is None or getattr(raised, "target_name", None) != expected_name:
        return False
    if pattern is None:
        return True
    ...
    return isinstance(pattern_value, str) and isinstance(message_value, str) \

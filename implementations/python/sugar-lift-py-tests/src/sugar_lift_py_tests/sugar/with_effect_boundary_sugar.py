...
class WithEffectBoundarySugar(Sugar):
    ...
    def desugar(self, ctx: object = None) -> Outcome:
        ...
        for manager_exit in manager_es.exits:
            ...
            disposition = AuthenticatedRaiseDisposition(expected, pattern)
            for body_exit in body_es.exits:
                if (
                    isinstance(body_exit, Halted)
                    and disposition_verdict(disposition, body_exit.effect) == "suppress"
                    and body_exit.state is None
                ):
                    raise SugarNotWritten(
                        owner="WithEffectBoundarySugar.desugar",
                        observed="raise face omitted its pre-effect state",
            ...
            completion = (
                HaltsCompletion(ExpectationNotMetEffect("raise", self.site))
                if isinstance(semantics.mode, ExpectsModeV1)
                else KeepsCompletion()
            )
            routed.append(
                body_es.and_exit(
                    ExitSet.completed(None),

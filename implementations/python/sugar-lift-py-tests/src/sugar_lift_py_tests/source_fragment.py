...
class SourceFragment:
    ...
    def from_source(cls, source: str, filename: str) -> "SourceFragment":
        ...
        # module per site. Sharing is sound for every consumer whose mutations
        # are idempotent node stamps (_sugar_* marks and the fragment caches);
        # the one consumer that structurally mutates its tree must use
        # from_source_private instead.
    ...
    @classmethod
    def from_source_private(cls, source: str, filename: str) -> "SourceFragment":
        """Parse Python source into a PRIVATE tree the caller may mutate.

        The one door for consumers that structurally mutate their tree (the
        install-source bridge clears decorator_list in place). A private parse
        keeps that mutation off the shared parsed_tree table; every read-only
        consumer must use from_source and ride the cache instead.
        """
    ...
    def own_scope_stored_names(self) -> "tuple[str, ...]":
        ...
        return LoopControlScopeSugar.own_scope_stored_names(self)
    ...
    def loop_carried_names(
        ...
        *,
        ...
        return LoopControlScopeSugar.loop_carried_names(
            self, target_name=target_name, entry_reads=entry_reads
        )
    ...
    def loop_stored_names(self) -> "tuple[str, ...]":
        ...
        return LoopControlScopeSugar.loop_stored_names(self)

    def has_loop_control(self) -> bool:
        """Delegate loop/control scope recognition to its owning Sugar."""
        from .sugar.loop_control_scope_sugar import LoopControlScopeSugar

        return LoopControlScopeSugar.classify(self).has_loop_control

    def has_unclassified_loop_mutation(self) -> bool:
        """Delegate loop/control scope recognition to its owning Sugar."""
        from .sugar.loop_control_scope_sugar import LoopControlScopeSugar

        return LoopControlScopeSugar.has_unclassified_loop_mutation(self)

    def while_definite_break_output_names(self) -> "tuple[str, ...]":
        """Delegate loop/control scope recognition to its owning Sugar."""
        from .sugar.loop_control_scope_sugar import LoopControlScopeSugar

        return LoopControlScopeSugar.while_definite_break_output_names(self)

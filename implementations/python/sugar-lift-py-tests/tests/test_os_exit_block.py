...
Expression statements evaluate and discard (Support is absorbed). Incomplete
contributes itself; unreached tail stays raw sugar.

    if True:
        True          -> Support (absorbed: expression statement discards)
        False         -> Support (absorbed)
...
def test_block_short_circuits_on_os_exit() -> None:
    ...
    # halted here -- earlier expression statements were Support and absorbed
    assert isinstance(record[0], Incomplete)
    assert isinstance(record[0].effect, OSExitRuntimeEffect)

    # unreachable: kept as raw, unreduced sugar -- NOT outcomes
    assert isinstance(record[1], SugarBody)
    assert isinstance(record[2], SugarBody)

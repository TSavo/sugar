...
def test_unbound_user_name_still_panics_loud() -> None:
    ...
    assert ax["silently_unaccounted"] == 0
    assert ax["refused_loud"] == 1 or ax["lifted_cited"] == 0
    fas = rpc.get("factoryAuditSummary") or {}
    # instrument engaged (unresolved) or refuse via accounting
    assert (
        int((fas.get("statusCounts") or {}).get("unresolved") or 0) > 0
        or ax["refused_loud"] == 1
    )

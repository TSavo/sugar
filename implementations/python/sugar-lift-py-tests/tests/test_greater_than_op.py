...
def test_greater_than_folds_to_true_when_greater() -> None:
    assert isinstance(_condition("if 2 > 1:\n    pass").value, TrueBoolLiteralSugar)


def test_greater_than_folds_to_false_when_equal() -> None:
    assert isinstance(_condition("if 2 > 2:\n    pass").value, FalseBoolLiteralSugar)


def test_greater_than_folds_to_false_when_less() -> None:

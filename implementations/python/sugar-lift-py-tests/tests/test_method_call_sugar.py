...
def test_method_call_reduces_to_call_method_coordinate() -> None:
    ...
    value = reduce_value(
        "z.m(3)", binds={"z": SymbolicValue(make_var("z"))}
    )
...
def test_owns_method_call_not_plain_name_or_os_exit() -> None:
    ...
    method_cands = [c.name for c in catalog.candidates_for(SugarRole.TERM, _site("z.m(1)"))]
    plain_cands = [c.name for c in catalog.candidates_for(SugarRole.TERM, _site("f(1)"))]
    exit_cands = [c.name for c in catalog.candidates_for(SugarRole.TERM, _site("os.exit(0)"))]
...
def test_zero_arg_method_call() -> None:
    value = reduce_value(
        "z.copy()", binds={"z": SymbolicValue(make_var("z"))}
    )

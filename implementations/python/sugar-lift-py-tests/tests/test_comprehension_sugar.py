...
def test_comprehension_runtime_iterables_refuse() -> None:
    ...
    for source, observed, selected in (
        ("{x: x for x in items}", "dict comprehension runtime boundary", "DictComp"),
        ("{x for x in items}", "set comprehension runtime boundary", "SetComp"),
    ):
        ...
        assert observed in outcome.reason
        assert f"{selected}.runtime_iterable" not in outcome.reason
        assert "runtime iterable `Name`" in outcome.reason
        assert "typed red" in outcome.reason
        assert "blame=" in outcome.reason


def test_runtime_comprehension_call_argument_records_typed_absence() -> None:
    report = build_literal_call_report(
        source=(
            ...
            "def test_arg(items):\n"
            "    assert keep() == 1\n"
            "    assert helper({k: v for k, v in items}) == 1\n"
        ),
        filename="comprehension.py",
        memento_file="comprehension.py",
    )

    assert report is not None
    assert len(report.payload.ir) >= 1
    assert any(
        "dict comprehension runtime boundary" in effect.effect.reason

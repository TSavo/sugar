...
**Every regression wants to become a type — but only to its honest ceiling.**
Every unit test that says "code should be shaped this way" is a missing type
wearing a test's clothes — a bug that wants to be seen by the compiler and is
stuck being seen by a test runner instead. The test is the confession; the type
is the absolution. A shape-assertion in a suite is the system admitting it knows
an invariant it has not yet told the compiler about — and everything the
compiler has not been told, someone must police forever, at runtime, in CI, with
eyeballs, fallibly. The compiler polices always, before the commit exists.
Worked example, same day it was written: 43 "refused" rows at lift were not a
census to ratchet down — they were 43 compiler errors waiting for the codomain
type to be narrowed to its two lawful constructors. The census tooth built that
morning was deleted the same evening, superseded by the type. Every census,
ratchet, and tooth is a larval stronger substrate; when one hatches, delete the
shell.

The qualifier is load-bearing. Promoting past the invariant's honest ceiling is
not rigor; it is a plausible-green lie with a better costume. If the substrate
can make the illegal state unconstructable, the guarding instrument must become
deletable: narrow the codomain, close the enum, move the obligation into a
constructor, protocol, visibility boundary, or type, and delete the shell that
used to watch it. If the domain is genuinely open — vendor grammar, dependency
release shape, package re-export idioms, non-exhaustive ASTs, corpus drift — no
local type can honestly close it. The terminal shape there is a membrane: a
content-addressed auditor that makes tomorrow's upstream offender loud the day
it arrives.

Two lies are forbidden because they are the same lie about where enforcement
lives. Do not delete an instrument unless a stronger substrate now carries the
law; that is the answer-first reflex, unfinished work declared green. Do not
keep an instrument that names no offender class and gives no retirement path;
that is the ceremony reflex, diligence theater decaying into liturgy. A
surviving auditor is legitimate only when it justifies its persistence by domain
openness, names the offender classes it can see, and gives each class a
retirement path. Instruments must testify or retire: if it sings, it names live
offenders and the next architecture; if it sleeps, its silence is measured
empty-set evidence; if it neither sings nor sleeps, it is decorative enforcement
and must be deleted or made real.
...
- Offload: the battleaxe family is `bin/bcargo` (cargo), `bin/brun` (arbitrary commands), and `bin/bpytest` (pytest) — same sync-then-remote-exec engine, per-checkout remote roots, real exit-code propagation. Heavy runs go to battleaxe, not the local machine: full `make test-python`-scale suites, broad pytest sweeps, corpus renders' test halves are `bpytest`/`brun` jobs; keep the local box for focused tests and edits. One loaded evening of six agents queueing release builds behind one laptop compiler is the failure mode this family deletes. Same disciplines for all three: never foreground-block on a cold run (background + poll the log), capture the real `$?` inline (never a trailing echo's), and grep the log for the actual `test result:`/failure lines before calling it green.

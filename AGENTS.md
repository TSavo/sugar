...
- Ratchet invariant: every long-term goal is a vector of named gaps plus floor invariants. A gap is a measured nonnegative count emitted by instrumentation; a floor is a safety invariant that must never regress, such as `false_discharges == 0`. Arrival means every named gap is zero and every floor still holds. Do not collapse distinct gaps into one number.
- Example gap split: for assertion lifting, `Delta` can mean the coverage gap (`unresolved`, not yet accounted) and `Gamma` can mean the proof gap (`undecided`, lifted but hollow). Destination is `Delta == 0 && Gamma == 0` under the floor `false_discharges == 0`: every locus is discharged with teeth or named-refused, nothing dark, nothing vacuous.
...
- Long-term goal: write a ratchet unit test for each named gap. The test pins the current checked-in number and asserts the next number is strictly better: for example `unresolved < 334`, `undecided < 12`, `no_facts < 78`, or `support == 0`. When the number improves, update the pinned threshold downward in the same PR. Never loosen a ratchet upward except in an explicit accounting-correction PR.
- Gap accounting: every PR states which gap it lowers and which floors it preserves. A coverage PR should either be proof-neutral by construction or honestly update the proof-gap ratchet. A proof PR should drain hollow/undecided obligations without weakening the coverage accounting.

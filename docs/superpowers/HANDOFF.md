...
- 85870 → worktree ~/provekit-wt/typed-pool-3, branch codex/typed-pool-6 → #3041 Slice 6 (reports/witnesses/CLI/fixtures, last 50 rows → STABLE ZERO + gate flip). Lane after: S7 compat cleanup; #3160 BridgePin after. #3041 auto-closed twice-risk: REOPENED by coordinator, keep "Part of".
- 85868 → worktree ~/provekit-wt/boundvar-floor, branch codex/object-floor (STACKED on merged raise-routing; NOTE an OLD remote branch codex/object-floor exists — push may need force) → #3017 item 6 (ObjectValue floor). Items 1-5 merged (#3206 BoundVar, #3210 raise routing). #3017 auto-closed prematurely, REOPENED by coordinator. Lane: items 7→10, then irterm-boundary #3191-3198 (S1 first, strict serial). assertion_lift frontier pinned R=36.
- 85873 → worktree ~/provekit-wt/pykit-lambda, branch codex/pykit-generatorexp → #3110 GeneratorExp. FLOOR-PROJECTION CAMPAIGN COMPLETE (#3150 CLOSED; S1-S7 all merged #3202/#3207/#3209/#3211/#3215/#3216/#3219; second engine DELETED; factory-spine auditor at stable zero; agreement gate ARMED in build_literal_call_report). Known cosmetic debt: duplicate bridge-chain+floor fact emission (call:h()==42 twice) persists post-deletion — documented in #3219 body, file an issue if it bites. Lane: recognizer tail #3110→#3111+ per audit plan order.
- STACKING PATTERN (works): when a worker's next task shares files with its own unmerged PR, branch the new work off that PR's branch; by PR time the base has merged and the new PR shows only new commits. If not yet merged, worker rebases onto origin/main and force-pushes after coordinator merges.

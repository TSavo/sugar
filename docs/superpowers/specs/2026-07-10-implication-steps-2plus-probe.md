# Probe: Implication steps 2+ (feed-fold Obligations) — 2026-07-10

**Canonical path for coordinator ruling.** Full probe; no proving-core edits.
...
## 9. FROZEN RULINGS (T, 2026-07-10) — implement against these

All three point the same way: **discharge is authoritative, seal is memoization-of-proven (never proof), enumerate stays honest.**

### D1 — OPTION A (frozen)

Enumerate-tree `CallSite::implication()` **STAYS `None`**. Enumerate-time cannot know link-time `post ⊃ pre`; minting there repeats the dead lift-time `evidenceTerm` category error (#3831). "Un-stub the tree method" was the wrong framing — **do not un-stub**. The feed-fold lives on the **beat-2 / runner (link-resolved) face** where both contracts exist. Leave `sugar.enumerate` honest.

### D2 — OPTION 1 prove-then-feed (frozen)

Soundness gate. **Do NOT seal-all-then-insert.** An ImplicationMemento in the pool means "discharged/proven" — that is the only thing that makes Tier 0c short-circuiting sound.

1. Derive the obligation; discharge on the runner (Tier 3 real solve / tactic).
2. **ONLY after real discharge**, mint + insert the ImplicationMemento as a reuse cache (federation-free by CID).
3. Mint-convention `verdict: "holds"` is **NOT** proof — never let presence-of-seal substitute for discharge.
4. Pattern: WitnessPool `get_or_seal(cid)` — compute-then-seal, never seal-without-compute. Warm = CID already in pool (proven); cold = discharge then seal.

### D3 — beat-2 SOLE authority (frozen)

Derive obligations from the same resolved contracts beat 1 already binds; discharge on beat-2/runner; seal proven edges for reuse. **Do NOT re-enable linker discharge as authoritative.** No dual opinions.

### Teeth (mandatory; D2 makes them possible)

| Twin | Path |
|------|------|
| Truthful (`post ⊃ pre` holds) | discharges → sealed → future runs hit Tier 0c → sat |
| Lying (post does not follow from pre) | FAILS discharge → **never sealed** → not in pool → Tier 0c Unknown → Tier 3 refutes → unsat/refuse |

If seal-all-then-insert were used (rejected), lying twin false-discharges at Tier 0c. Bad-twin flip is the gate.

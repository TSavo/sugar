...
2. **Verdict/byte-compat is the floor on every implementation slice.** `CompiledFormula` output equality (except declared frontend-provenance metadata, which must be explicitly listed the moment it exists) across the refactor, demonstrated per slice with a planted-drift control; solver verdicts on the golden corpus unchanged. A deliberate output change files a soundness issue first.
...
| `R(frontend-provenance-metadata-undeclared)` | 0 (none exists yet) | stays 0 — any provenance metadata added by a frontend is declared the same PR or red |
...
- Bad-twins: (a) JSON-decoded and binary-decoded `CompilerInput` typed-equal on the fixture corpus; (b) a corrupted binary fixture refuses at decode; (c) `git diff --stat` of the slice shows zero lines in `sugar-ir-compiler-{smt-lib,lean,coq,maude}` (assert it in the PR body — this IS the proof).
...
- **No silent output drift.** `CompiledFormula` equality except declared provenance metadata; solver verdicts conserved; planted-drift control in every implementation slice.

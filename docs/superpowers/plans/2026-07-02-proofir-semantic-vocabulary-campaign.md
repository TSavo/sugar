...
> **Slices 5-10 are the deepened Construction-Law phase — the "ProofIR Construction Law" plan/spec, red-first (T Savo directive, design doc Addendum 3b, AUTHORITATIVE).** S4 (`EqualityFact` typed node at the seat + #3220, PR #3291 MERGED) is a COMPATIBLE step that stays as-is — T calls the current state "halfway there": it constructs `EqualityFact` with a `euf_key: str` per the S2 shape. The Construction-Law drain OWNS removing that param (S5 rebuilds `EqualityFact` to take `CallTerm[S]` + `Term[S]` with the key DERIVED). The directive EXTENDS beyond S4 into typed FOL MEMBERSHIP. T's drain order (authoritative): scanner + failing construction tests → `EqualityFact` → `FunctionContract` → `RefusalRecord` → remaining nodes → close.

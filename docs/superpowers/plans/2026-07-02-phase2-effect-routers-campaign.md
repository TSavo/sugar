...
## Interface authority (typed-pipeline specs, PR #3312)

The effect/router surfaces meet two typed seams (design rule: typed boundaries are the substrate; JSON is a transport format). The verdict acceptance — the TrySugar witness lying-twin going UNSAT (Instrument C) and the byte-compat harness (Instrument B) — rides `docs/superpowers/specs/2026-07-02-ir-compiler-solver-interface.md` (`CompiledFormula` → `Solver::solve_compiled` → `SolveResult.verdict`; the router's soundness is judged there, and `CompiledFormula.opacity_manifest` is where an unrouted/opaque control-flow position must NOT silently read as discharged — invariant §1 "compiler opacity is load-bearing"). The lift driver that feeds the router (`lift_file` → contract emission) is governed by `docs/superpowers/specs/2026-07-02-lift-plugin-interface.md`. The exposed spine (Slice 3) is the algebra surface that irterm Slice 6 (#3196) consumes through the boundary; the term representation both cross is the seam the interface map flags as migration debt.

#!/usr/bin/env bash
# stdlib encode_nopad logo dual-assert receipt (CI ratchet) + scope boundary.
#
# Ambient: closed strip not-suffix-of("=", out) on call:itsdangerous.encoding.encode_nopad
# (grounding path #3939 / #3949 / #3952 / #3956). Same #euf# key:
#
#   GOOD:            unpadded b"cHJvdmVraXQ"  -> encode_nopad consistency discharged
#   BAD:             padded   b"cHJvdmVraXQ=" -> encode_nopad consistency unsatisfied
...
run_logo_twin() {
  ...
  echo
  echo "==================== logo twin: $twin (expect encode_nopad: $expect) ===================="
...
found = [
    ...
    if "encode_nopad" in str(r.get("property", ""))
]
if not found:
    print(f"FAIL({twin}): no encode_nopad rows in prove receipt")
    sys.exit(1)

print(f"encode_nopad rows({twin}):")
for r in found:
...
if not ok:
    print(f"FAIL({twin}): expected encode_nopad {expect}, statuses={sorted(statuses)}")
    sys.exit(1)

if expect == "unsatisfied" and "unsatisfied" not in statuses and statuses & bad_words:
    print(
        f"WARN({twin}): encode_nopad is unsat-family {sorted(statuses)} "
        f"but not literal 'unsatisfied'"
    )

print(f"OK({twin}): encode_nopad {expect}")
INNER
}
